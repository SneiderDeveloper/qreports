<template>
  <div>
    ...
    <h4 class="text-primary tw-text-base tw-font-extrabold tw-mb-6">
      Select the info you want to show for each filter
    </h4>
    <div
      class="
        tw-grid tw-gap-x-8 tw-gap-y-6 tw-grid-cols-1
        md:tw-grid-cols-2
        lg:tw-grid-cols-3
        tw-my-4 tw-overflow-hidden
      "
    >
      <div v-for="(field, keyField) in formFields.reportsForms" :key="keyField">
        <dynamic-field
...
export default {
  ...
  computed: {
    ...
    formFields() {
      return {
        reportsForms: fieldsDetailsStore().factoryOfDynamicSelect(),
      };
    },
  },
};
